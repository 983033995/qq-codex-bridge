...
5. Admin 与 MCP 共用同一套 Application Service，不允许出现两套配置与业务逻辑。
6. Codex AppServer 保持主链路；CDP 仅作为单会话、安全的 pre-submit recovery。
7. Approval、Turn Ledger、Runtime 状态和渠道状态持久化，提升长时间运行可靠性。
...
4. Approval
5. Durable Turn Ledger + Transport Isolation
6. Control Center UI
7. Release Hardening
...
> 将旧的 qq-codex-bridge 升级为 **OmniAgent Gateway**：一个配置 MCP 后即可由 AI 自动初始化和运行，并通过 QQ、微信、飞书接收消息，经智能路由后驱动 Codex 或执行系统控制的本地 Agent Gateway。
