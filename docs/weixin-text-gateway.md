...
- 微信**文本** long-poll 入站，以及语音消息的转写文本（`voice_item.text`）
- bridge 文本 / 图片 / 视频 / 文件 / 音频附件出站（iLink 会静默丢弃主动 `voice_item`，所以音频走 `file_item`；媒体统一通过 `ilink/bot/getuploadurl` + AES-128-ECB 加密 CDN 上传，并与桥接内部 `MediaArtifact` 协议对接）
- 扫码登录与本地状态持久化
- 本地联调可观测性

还没有覆盖：

- **入站图片、视频、文件的真正下载**——收到这类消息时，网关会识别出消息类型并回复一段占位说明文字（例如"当前微信网关暂不支持下载入站图片"），让用户知道消息已收到但内容无法读取，而不是像早期版本那样直接静默丢弃；真正的下载/解密需要先从真实账号抓包确认入站媒体的字段格式（`ilink/bot` 协议目前没有公开文档说明这部分）
- 群聊 `@bot`
- 富媒体卡片

所以它当前更适合作为：

- 微信私聊文本桥接，且可以把 Codex 生成的图片/视频/文件/音频附件正常回传给用户
- long-poll 基线实现
- 后续"入站媒体下载"扩展的起点（扩展前建议先用真实账号发一次图片/文件，从 `[weixin-gateway] inbound media placeholder used` 日志里确认 `item_list` 的真实字段，再实现解密下载）
