...
const errorDispositions: Record<StableErrorCode, ErrorDisposition> = {
  ...
  CODEX_TURN_QUEUE_FULL: disposition(true, false, false, "Wait for queue capacity before retrying"),
  CODEX_TURN_CANCELLED: disposition(false, false, false, "Submit a new message if the work is still needed"),
  CODEX_TURN_RECOVERY_FAILED: disposition(false, true, false, "Inspect the unknown turn before retrying"),
  CODEX_TURN_TIMEOUT: disposition(true, false, false, "Inspect the turn before retrying"),
  MESSAGE_SEQUENCE_CONFLICT: disposition(false, false, false, "Discard the duplicate or stale inbound message"),
